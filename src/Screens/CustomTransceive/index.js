...
function CustomTransceiveScreen(props) {
  ...
  const [commands, setCommands] = React.useState(params.savedRecord?.payload.value || []);
  ...
  const hasCustomExecuteFunc = typeof params.savedRecord?.onExecute === 'function';
  ...
  async function executeCommands() {
    ...
    try {
      ...
      if (nfcTech === NfcTech.NfcA) {
        result = await NfcProxy.customTransceiveNfcA(commands, params.savedRecord?.onPostExecute);
      } else if (nfcTech === NfcTech.NfcV) {
        result = await NfcProxy.customTransceiveNfcV(commands, params.savedRecord?.onPostExecute);
      } else if (nfcTech === NfcTech.IsoDep) {
        result = await NfcProxy.customTransceiveIsoDep('Custom TransceiveIsoDep')(
          commands,
    ...
    if (!success) {
      Alert.alert('Commands Not Finished', '', [{text: 'OK', onPress: () => 0}]);
    }
        ...
        {editableParameters && (
          <View style={{padding: 10}}>
            ...
            {editableParameters.map((p, idx) => {
              const hexString = p.payload.map((b) => ('00' + b.toString(16)).slice(-2)).join(' ');
              return (
                <View
                  style={{
                    flexDirection: 'row',
            ...
            <ScrollView style={[styles.wrapper, {padding: 10}]}>
              {commands.map((cmd, idx) => (
                <CommandItem
                  ...
                  resp={responses[idx]}
                  key={idx}
                  onMoveUp={commands.length > 1 && idx > 0 && (() => moveCommandUp(idx))}
                  onMoveDown={
                    commands.length > 1 && idx < commands.length - 1 && (() => moveCommandDown(idx))
                  }
