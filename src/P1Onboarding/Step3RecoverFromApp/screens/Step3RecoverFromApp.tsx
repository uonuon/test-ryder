...
import {getStoredBackupSharePhone} from '../../../../lib/storage';
import {sendBackupShare} from '../../../../lib/nfc-protocol/wallet';

const Step3RecoverFromApp = ({route, navigation}) => {
  const {navigate, goBack} = navigation;
  const ryderOneAnimation = require('../assets/tap_ryder_one.mp4');

  const beginRecovery = async () => {
    // TODO: nfc protocol "send backup share"
    const share = await getStoredBackupSharePhone();
    if (share) {
      const result = await sendBackupShare(share);
      navigate('Step4RecoverFromApp');
    } else {
      Alert.alert('TapSafe Error', 'No TapSafe Backup found on this phone.');
    }
  };
      ...
      <View style={[styles.welcome, styles.welcomeFlexBox]}>
        <View style={[styles.frameParent, styles.welcomeFlexBox]}>
          <View style={[styles.frameGroup, styles.frameWrapperFlexBox]}>
            <ScreenheaderOnboarding step={3} mode={'recover'} />
            <View style={[styles.frameWrapper, styles.frameWrapperFlexBox]}>
              <View style={[styles.frameView, styles.frameSpaceBlock]}>
                <View>
                  <Text style={[styles.step4, styles.step4Typo]}>Step 3</Text>
                  <Text style={[styles.tapRyderOne, styles.labelTextTypo]}>
                    Tap to send first backup
                  </Text>
                </View>
                <Text style={[styles.ryderProductsWork, styles.step4Typo]}>
                  Press “Ready to scan” and hold your Ryder One near your phone to begin recovery.
                </Text>
              </View>
            </View>
          </View>
...
export default Step3RecoverFromApp;
